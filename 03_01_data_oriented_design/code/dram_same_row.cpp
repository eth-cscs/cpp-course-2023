...
struct OffsetGenerator {
    OffsetGenerator(size_t rangeSize, size_t rowSize)
        : rowSize(rowSize),
...
int main() {
    ...
    std::unique_ptr<int64_t, void (*)(void*)> data{
        static_cast<int64_t*>(std::aligned_alloc(16384, numValues * sizeof(int64_t))),
    ...
    for (size_t rowSize = 1; rowSize <= 4096; rowSize *= 2) {
        OffsetGenerator gen{ values.size() / burstSize, rowSize };
        int64_t result = 0;
